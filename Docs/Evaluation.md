...
The number worth flagging here is **TN = 0**: TP + FP = 10 means the RAG path predicted
`isFlagged = true` for *every single file in the set*, including all 4 benign ones —
recall is 100% only because nothing was ever predicted negative, not because the rule is
correctly discriminating benign from over-privileged content. At `temperature: 0` this
run's precision (60%) still clears the 0.5 bar, but a rule that flags everything would
pass this specific threshold regardless of accuracy — the eval set doesn't yet have a
test that would catch "always flag" as a degenerate failure mode. Worth adding a check
for TN > 0 (or a minimum specificity) once the benign bucket grows past 4 samples.
